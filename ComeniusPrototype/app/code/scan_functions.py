...
class ScanFunctions:
    ...
    def split_grade_ancova(self, text:str, solution:dict) -> str:
        if self.mes['L_ENGLISH']:
            ...
            nl_nlp = spacy.load('nl_core_news_sm')
        doc = nl_nlp(text.lower())
        markers = ['predictive value',('predictive')] if self.mes['L_ENGLISH'] else ['voorspellende waarde',('voorspellend','voorspellende')]
        ...
        between_sent = [x for x in doc.sents if (lef(solution['independent'].get_all_syns(),[y.text for y in x]) or 'between-subject' in x.text) and ('significant' in x.text or 'effect' in x.text) and not any([y.text in markers[1] for y in x])]
        if between_sent != []:
