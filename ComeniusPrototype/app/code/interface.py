...
class Controller:
    ...
    def update(self, textfields: Dict) -> str:
        ...
        #Choosing an analysis:
            ...
            if analysis in ['Multiple regressieanalyse', 'Multiple regression analysis']:
            ...
            if analysis == 'Multivariate RMANOVA':
                ...
                if report not in ['Beknopt rapport', 'Short report']:
                    return self.protocol[self.index][0] + '<br><span style="color: blue;">'+self.mes['M_APOLOGIES']+analysis.replace('Multi','multi')+self.mes['S_SHORTONLY']+'</span>'
            if analysis in ['Dubbel Multivariate-RMANOVA','Double multivariate RMANOVA']:
            ...
            #Select report type
            self.index = 0
            if report in ['Elementair rapport (oefenmodus)','Basic report (practice mode)']:
                self.skipable = True
            ...
            if report in ['Elementair rapport (tentamenmodus)','Basic report (examination mode)']:
                self.submit_field = Task.FINISHED #Task.INTRO
                self.skipable = False
                self.formmode = True
                self.protocol = self.completion_protocol()
            if report in ['Beknopt rapport', 'Short report']:
